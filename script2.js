document.addEventListener("DOMContentLoaded", () => {
    ...
    const reelVideos = Array.from(document.querySelectorAll('.reel-video-wrapper video'));
    ...
    function updateReelPlayback() {
        ...
        const activeIndex = Math.max(0, Math.min(reelVideos.length - 1, Math.round(state.virtualActive)));
    ...
    function renderLayout() {
        ...
        for (let i = 0; i < reelItems.length; i++) {
            ...
            const wrapperW = maxW * currentScale + (maxW * 1.5 - maxW) * centerWeight * state.progress;
            const wrapperH = maxH * currentScale + (maxH * 1.5 - maxH) * centerWeight * state.progress;
            ...
            positions.push({ wrapperW, wrapperH, itemScale, itemY, blurAmount, opacityAmount, trueW, zIndex });
            
            if (i === 0) {
                xOffsets[0] = 0;
            } else {
                xOffsets[i] = xOffsets[i - 1] + positions[i - 1].trueW / 2 + gap + trueW / 2;
            }
        }
        ...
        gsap.set(".video-overlay", { backgroundColor: `rgba(0,0,0,${0.2 + 0.75 * state.progress})` });
        gsap.set(".bottom-reels", { y: -window.innerHeight * 0.28 * state.progress });

        const uiAlpha = Math.max(0, (state.progress - REEL_PLAY_THRESHOLD) / (1 - REEL_PLAY_THRESHOLD));
        gsap.set(".bottom-right-details", { opacity: uiAlpha, y: 20 * (1 - uiAlpha) });
        gsap.set(".slider-controls", { opacity: uiAlpha });

        updateReelPlayback();
    }

    gsap.set(track, { position: 'relative', display: 'flex', alignItems: 'center', justifyContent: 'center' });
    reelItems.forEach(item => {
        gsap.set(item, { position: 'absolute', top: '50%', left: '50%', xPercent: -50, yPercent: -50 });
        const wrapper = item.querySelector('.reel-video-wrapper');
        item.addEventListener('mouseenter', () => {
            gsap.to(wrapper, { scale: 1.05, duration: 0.3, ease: 'power2.out', overwrite: 'auto' });
        });
    ...
    function initScrollAndControls() {
        gsap.to(".hero-title", {
            ...
            x: () => {
                ...
                const source = document.querySelector('.hero-title').getBoundingClientRect();
        ...
        if (reelItems.length) {
            ...
            function updateDetails(index) {
                const descH3 = reelItems[index].querySelector('.reel-description h3').innerText;
            ...
            prevBtn.addEventListener('click', () => {
                ...
                gsap.to(state, { virtualActive: target, duration: 0.5, ease: "power2.out", onUpdate: renderLayout });
                updateDetails(target);
            });

            nextBtn.addEventListener('click', () => {
                const target = Math.min(reelItems.length - 1, Math.round(state.virtualActive) + 1);
                gsap.to(state, { virtualActive: target, duration: 0.5, ease: "power2.out", onUpdate: renderLayout });
                updateDetails(target);
            });
        }
        ...
        if (expertiseSection) {
            ...
            tl.to('.expertise-cube', { rotateX: -90, duration: 1, ease: 'power2.inOut' });
            tl.to('.exp-bg-1', { opacity: 0, duration: 1, ease: 'power2.inOut' }, "<");
            tl.to('.exp-bg-2', { opacity: 1, duration: 1, ease: 'power2.inOut' }, "<");
            tl.to('.exp-dot-1', { height: '4px', backgroundColor: '#fff', duration: 0.5 }, "<");
            tl.to('.exp-dot-2', { height: '24px', backgroundColor: '#ff6b35', duration: 0.5 }, "<");

            tl.to('.expertise-cube', { rotateX: -180, duration: 1, ease: 'power2.inOut' });
            tl.to('.exp-bg-2', { opacity: 0, duration: 1, ease: 'power2.inOut' }, "<");
            tl.to('.exp-bg-3', { opacity: 1, duration: 1, ease: 'power2.inOut' }, "<");
            tl.to('.exp-dot-2', { height: '4px', backgroundColor: '#fff', duration: 0.5 }, "<");
            tl.to('.exp-dot-3', { height: '24px', backgroundColor: '#ff6b35', duration: 0.5 }, "<");

            tl.to('.expertise-cube', { rotateX: -270, duration: 1, ease: 'power2.inOut' });
            tl.to('.exp-bg-3', { opacity: 0, duration: 1, ease: 'power2.inOut' }, "<");
            tl.to('.exp-bg-4', { opacity: 1, duration: 1, ease: 'power2.inOut' }, "<");
            tl.to('.exp-dot-3', { height: '4px', backgroundColor: '#fff', duration: 0.5 }, "<");
            tl.to('.exp-dot-4', { height: '24px', backgroundColor: '#ff6b35', duration: 0.5 }, "<");
            tl.to('.exp-bg', { scale: 1.1, duration: tl.duration(), ease: 'none' }, 0);
    ...
    function prepareIntroLetters() {
        ...
        const alreadyPrepared = introScriptTitle.querySelectorAll(".intro-script-letter");
    ...
    function runIntroSequence() {
        ...
        introTl
            ...
            .to(".bottom-reels", { opacity: 1, duration: 1.2, ease: "power2.out" }, "<");
    }
